...
def can_book(points: int, requested: int) -> bool:
    """Business rule: 1..12 places and enough points."""
    return 1 <= requested <= 12 and points >= requested


def sanitize_places(value) -> int:
    """Cast user input to positive int or raise ValueError."""
    try:
...
def calculate_remaining_places(total: int, booked: int) -> int:
    """Compute remaining places, validate bounds."""
...
# Eager-load JSON data once (tests patch paths when needed)
...
def book(competition, club):
    """Open booking page for a given competition/club."""
...
def purchasePlaces():
    """Validate and process a booking request."""
    competition_name = request.form.get("competition", "")
    club_name = request.form.get("club", "")
    raw_places = request.form.get("places", "")
    ...
    try:
        places_required = sanitize_places(raw_places)
    except ValueError:
        places_required = 0
    ...
    if not can_book(club_points, places_required):
...
@app.route("/points")
def public_points():
    """Public endpoint: list clubs and points as JSON."""
